...
//      var $column_order = array('job.id',null,null); //set column field database for datatable orderable

//     var $column_search = array('job.id','job.job_number'); //set column field database for datatable searchable just firstname , lastname , address are searchable

//     var $order = array('job.id' => 'asc');
...
//     }
  ...
  {
        $this->db->select('job_remarks.*,job.job_number');
        $this->db->from($this->table);
       $this->db->join('job', 'job.id = job_remarks.job_id');
        $this->db->where('job_remarks.user_id',$this->rankfordAdminDetails->id);
         if($job_id !=''){
