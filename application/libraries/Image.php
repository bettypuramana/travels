...
class Image {
    ...
    function generalCropAdd()
    {
        ...
        {
        $iWidth = 720;
        $iHeight = 335; // desired image result dimensions
        }
    ...
    function imageCropAdd_institute()
    {

    $ci_name=strtolower($this->CI->router->fetch_class());

     if (!is_dir('uploads/institutes/'.$ci_name))
     {
        mkdir('uploads/institutes/'.$ci_name, 0755, TRUE);
        mkdir('uploads/institutes/'.$ci_name.'/crop', 0755, TRUE);
     }

        $config['upload_path'] = 'uploads/institutes/'.$ci_name.'/';
        $config['allowed_types'] = 'gif|jpg|png|bmp|jpeg|pdf';
        $config['file_name'] = $_FILES['image_file']['name'];
        $config['max_size'] = '0';
        $config['max_width'] = '0';
        ...
        if (is_uploaded_file($_FILES['image_file']['tmp_name'])) {
        // new unique filename
        $sTempFileName = 'uploads/institutes/'.$ci_name.'/crop/' . md5(time() . rand());
        // move uploaded file into cache folder
        move_uploaded_file($_FILES['image_file']['tmp_name'], $sTempFileName);
        // change file permission to 644
        @chmod($sTempFileName, 0644);
        ...
        // output image to file
        $crop_image = explode("/", $sResultFileName);
        $this->crop_image_name = $crop_image[4];
        imagejpeg($vDstImg, $sResultFileName, $iJpgQuality);
        @unlink($sTempFileName);
        }
        }
    ...
    function generalCropAdd_institute()
    {
